...
usb_audio_type audio_struct = {0, 0, 0, 0, 0, 0x1400, 0, 0, 0,
                               {0x0000, 0x1400, 0x33},
                               {AUDIO_MIC_VOLUME_MIN, AUDIO_MIC_VOLUME_MAX, AUDIO_MIC_VOLUME_RES}, 0, 0};
...
static usb_sts_type class_init_handler(void *udev)
{
  ...
  paudio->mic_freq = AUDIO_DEFAULT_FREQ;
  ...
  paudio->spk_freq = AUDIO_DEFAULT_FREQ;
    ...
    switch( paudio->request_no)
    {
      case AUDIO_VOLUME_CONTROL:
        if( paudio->interface == AUDIO_SPK_FEATURE_UNIT_ID)
        {
          ...
          /* set volume */
          usbd_audio_set_spk_volume(paudio->spk_volume*100/paudio->spk_volume_limits[1]);
        }
        else
        {
          paudio->mic_volume = (uint16_t)(paudio->g_audio_cur[0] |
                                               (paudio->g_audio_cur[1] << 8));
          {
            int32_t volume = (int16_t)paudio->mic_volume;
            int32_t min_volume = (int16_t)paudio->mic_volume_limits[0];
            int32_t max_volume = (int16_t)paudio->mic_volume_limits[1];
            int32_t gain = (volume - min_volume) * 0x100 / (max_volume - min_volume);
            usbd_audio_set_mic_volume((uint16_t)gain);
          }
        }
        break;
      case AUDIO_MUTE_CONTROL:
        if(paudio->interface == AUDIO_SPK_FEATURE_UNIT_ID)
        {
          ...
          usbd_audio_set_spk_mute(paudio->spk_mute);
        }
        else
        {
          paudio->mic_mute = paudio->g_audio_cur[0];
          usbd_audio_set_mic_mute(paudio->mic_mute);
        }
      ...
      case AUDIO_FREQ_SET_CONTROL:
        if(paudio->enpd == USBD_AUDIO_MIC_IN_EPT)
        {
         paudio->mic_freq = (paudio->g_audio_cur[0] |
          ...
          usbd_audio_set_mic_freq(paudio->mic_freq);
        }
        else
        {
          paudio->spk_freq = (paudio->g_audio_cur[0] |
                            (paudio->g_audio_cur[1] << 8) |
                            (paudio->g_audio_cur[2] << 16));
          usbd_audio_set_spk_freq(paudio->spk_freq);
        }
        break;
      default:
        break;
...
static usb_sts_type class_in_handler(void *udev, uint8_t ept_num)
{
  ...
  */
  ...
  {
    len = usbd_audio_mic_get_data(paudio->audio_mic_data);
    usbd_ept_send(udev, USBD_AUDIO_MIC_IN_EPT, paudio->audio_mic_data, len);
  }

    else if((AUDIO_SUPPORT_FEEDBACK == 1) &&
      (ept_num & 0x7F) == (USBD_AUDIO_FEEDBACK_EPT & 0x7F))
  {
    len = usbd_audio_spk_feedback(paudio->audio_feed_back);
...
static usb_sts_type class_out_handler(void *udev, uint8_t ept_num)
{
  ...
  {
    /* speaker data*/
    usbd_audio_spk_put_data(paudio->audio_spk_data, g_rxlen);
...
usb_sts_type class_event_handler(void *udev, usbd_event_type event)
{
  ...
  switch(event)
  {
    ...
    case USBD_SUSPEND_EVENT:
      ...
      usbd_audio_spk_alt_setting(paudio->spk_alt_setting);
      paudio->mic_alt_setting = 0;
      usbd_audio_mic_alt_setting(paudio->spk_alt_setting);
      /* ...user code... */
...
static void audio_set_interface(void *udev, usb_setup_type *setup)
{
  ...
  if(LBYTE(setup->wIndex) == AUDIO_SPK_INTERFACE_NUMBER)
  {
    ...
    {
      int len = usbd_audio_spk_feedback(paudio->audio_feed_back);
      usbd_ept_recv(pudev, USBD_AUDIO_SPK_OUT_EPT, paudio->audio_spk_data, AUDIO_SPK_OUT_MAXPACKET_SIZE);
  ...
  else if(LBYTE(setup->wIndex) == AUDIO_MIC_INTERFACE_NUMBER)
  {
    ...
    usbd_audio_mic_alt_setting(paudio->mic_alt_setting);
