/**
  **************************************************************************
  * @file     cdc_custom_hid_desc.c
  * @brief    usb cdc + custom hid composite device descriptor
  **************************************************************************
  *
  * Copyright (c) 2025, Artery Technology, All rights reserved.
  *
  * The software Board Support Package (BSP) that is made available to
  * download from Artery official website is the copyrighted work of Artery.
  * Artery authorizes customers to use, copy, and distribute the BSP
  * software and its related documentation for the purpose of design and
  * development in conjunction with Artery microcontrollers. Use of the
  * software is governed by this copyright notice and the following disclaimer.
  *
  * THIS SOFTWARE IS PROVIDED ON "AS IS" BASIS WITHOUT WARRANTIES,
  * GUARANTEES OR REPRESENTATIONS OF ANY KIND. ARTERY EXPRESSLY DISCLAIMS,
  * TO THE FULLEST EXTENT PERMITTED BY LAW, ALL EXPRESS, IMPLIED OR
  * STATUTORY OR OTHER WARRANTIES, GUARANTEES OR REPRESENTATIONS,
  * INCLUDING BUT NOT LIMITED TO, THE IMPLIED WARRANTIES OF MERCHANTABILITY,
  * FITNESS FOR A PARTICULAR PURPOSE, OR NON-INFRINGEMENT.
  *
  **************************************************************************
  */
#include "usb_std.h"
#include "usbd_sdr.h"
#include "usbd_core.h"
#include "cdc_custom_hid_desc.h"
#include "cdc_custom_hid_class.h"

#ifdef USB_USE_CDC_CUSTOM_HID

/** @defgroup USB_cdc_custom_hid_desc
  * @brief usb device cdc + custom hid composite descriptor
  * @{
  */

/** @defgroup USB_cdc_custom_hid_desc_private_functions
  * @{
  */
static usbd_desc_t *get_device_descriptor(void);
static usbd_desc_t *get_device_qualifier(void);
static usbd_desc_t *get_device_configuration(void);
static usbd_desc_t *get_device_other_speed(void);
static usbd_desc_t *get_device_lang_id(void);
static usbd_desc_t *get_device_manufacturer_string(void);
static usbd_desc_t *get_device_product_string(void);
static usbd_desc_t *get_device_serial_string(void);
static usbd_desc_t *get_device_interface_string(void);
static usbd_desc_t *get_device_config_string(void);

static uint16_t usbd_unicode_convert(uint8_t *string, uint8_t *unicode_buf);
static void usbd_int_to_unicode (uint32_t value , uint8_t *pbuf , uint8_t len);
static void get_serial_num(void);
#if defined ( __ICCARM__ ) /* iar compiler */
  #pragma data_alignment=4
#endif
ALIGNED_HEAD static uint8_t g_usbd_desc_buffer[256] ALIGNED_TAIL;

/**
  * @brief composite device descriptor handler structure
  */
usbd_desc_handler cdc_custom_hid_desc_handler =
{
  get_device_descriptor,
  get_device_qualifier,
  get_device_configuration,
  get_device_other_speed,
  get_device_lang_id,
  get_device_manufacturer_string,
  get_device_product_string,
  get_device_serial_string,
  get_device_interface_string,
  get_device_config_string,
};

/**
  * @brief usb device standard descriptor
  *        class/subclass/protocol = 0: class defined at interface level (composite)
  */
#if defined ( __ICCARM__ ) /* iar compiler */
  #pragma data_alignment=4
#endif
ALIGNED_HEAD static uint8_t g_usbd_descriptor[USB_DEVICE_DESC_LEN] ALIGNED_TAIL =
{
  USB_DEVICE_DESC_LEN,                   /* bLength */
  USB_DESCIPTOR_TYPE_DEVICE,             /* bDescriptorType */
  0x00,                                  /* bcdUSB */
  0x02,
  0xEF,                                  /* bDeviceClass: composite with IAD (USB-IAD 1.0) */
  0x02,                                  /* bDeviceSubClass */
  0x01,                                  /* bDeviceProtocol */
  USB_MAX_EP0_SIZE,                      /* bMaxPacketSize */
  LBYTE(USBD_VENDOR_ID),  /* idVendor */
  HBYTE(USBD_VENDOR_ID),  /* idVendor */
  LBYTE(USBD_PRODUCT_ID), /* idProduct */
  HBYTE(USBD_PRODUCT_ID), /* idProduct */
  0x00,                                  /* bcdDevice rel. 2.00 */
  0x02,
  USB_MFC_STRING,                        /* Index of manufacturer string */
  USB_PRODUCT_STRING,                    /* Index of product string */
  USB_SERIAL_STRING,                     /* Index of serial number string */
  1                                      /* bNumConfigurations */
};

/**
  * @brief usb configuration standard descriptor
  *        interface0: cdc control (int ep 0x82)
  *        interface1: cdc data    (bulk ep 0x81 / 0x01)
  *        interface2: custom hid  (int ep 0x83 / 0x03)
  */
#if defined ( __ICCARM__ ) /* iar compiler */
  #pragma data_alignment=4
#endif
ALIGNED_HEAD static uint8_t g_usbd_configuration[USBD_CDC_CUSTOM_HID_CONFIG_DESC_SIZE] ALIGNED_TAIL =
{
  USB_DEVICE_CFG_DESC_LEN,               /* bLength: configuration descriptor size */
  USB_DESCIPTOR_TYPE_CONFIGURATION,      /* bDescriptorType: configuration */
  LBYTE(USBD_CDC_CUSTOM_HID_CONFIG_DESC_SIZE),  /* wTotalLength: bytes returned */
  HBYTE(USBD_CDC_CUSTOM_HID_CONFIG_DESC_SIZE),  /* wTotalLength: bytes returned */
  0x03,                                  /* bNumInterfaces: 3 interfaces */
  0x01,                                  /* bConfigurationValue: configuration value */
  0x00,                                  /* iConfiguration */
  0xC0,                                  /* bmAttributes: self powered */
  0x32,                                  /* MaxPower 100 mA */

  /* ------------- IAD: bind cdc ctrl (if0) + cdc data (if1) as one function ------------- */
  USB_DEVICE_IAD_DESC_LEN,               /* bLength: IAD size 8 */
  USB_DESCIPTOR_TYPE_IAD,                /* bDescriptorType: interface association 0x0B */
  0x00,                                  /* bFirstInterface: 0 */
  0x02,                                  /* bInterfaceCount: if0 + if1 */
  USB_CLASS_CODE_CDC,                    /* bFunctionClass: CDC */
  0x02,                                  /* bFunctionSubClass: ACM */
  0x01,                                  /* bFunctionProtocol: AT commands */
  0x00,                                  /* iFunction */

  /* ------------- interface 0: cdc communication ------------- */
  USB_DEVICE_IF_DESC_LEN,                /* bLength: interface descriptor size */
  USB_DESCIPTOR_TYPE_INTERFACE,          /* bDescriptorType: interface descriptor type */
  0x00,                                  /* bInterfaceNumber: interface 0 */
  0x00,                                  /* bAlternateSetting: alternate set */
  0x01,                                  /* bNumEndpoints: 1 endpoint */
  USB_CLASS_CODE_CDC,                    /* bInterfaceClass: CDC class code */
  0x02,                                  /* bInterfaceSubClass: Abstract Control Model */
  0x01,                                  /* bInterfaceProtocol: AT Command */
  0x00,                                  /* iInterface */

  0x05,                                  /* bFunctionLength */
  USBD_CDC_CS_INTERFACE,                 /* bDescriptorType: CDC interface descriptor type */
  USBD_CDC_SUBTYPE_HEADER,               /* bDescriptorSubtype: header function descriptor */
  LBYTE(CDC_CUSTOM_HID_BCD_NUM),
  HBYTE(CDC_CUSTOM_HID_BCD_NUM),         /* bcdCDC */

  0x05,                                  /* bFunctionLength */
  USBD_CDC_CS_INTERFACE,                 /* bDescriptorType: CDC interface descriptor type */
  USBD_CDC_SUBTYPE_CMF,                  /* bDescriptorSubtype: call management */
  0x00,                                  /* bmCapabilities */
  0x01,                                  /* bDataInterface: data interface 1 */

  0x04,                                  /* bFunctionLength */
  USBD_CDC_CS_INTERFACE,                 /* bDescriptorType: CDC interface descriptor type */
  USBD_CDC_SUBTYPE_ACM,                  /* bDescriptorSubtype: abstract control management */
  0x02,                                  /* bmCapabilities: set/get line coding */

  0x05,                                  /* bFunctionLength */
  USBD_CDC_CS_INTERFACE,                 /* bDescriptorType: CDC interface descriptor type */
  USBD_CDC_SUBTYPE_UFD,                  /* bDescriptorSubtype: union functional descriptor */
  0x00,                                  /* bControlInterface 0 */
  0x01,                                  /* bSubordinateInterface0: interface 1 */

  USB_DEVICE_EPT_LEN,                    /* bLength: endpoint descriptor size */
  USB_DESCIPTOR_TYPE_ENDPOINT,           /* bDescriptorType: endpoint descriptor type */
  USBD_CDC_INT_EPT,                      /* bEndpointAddress: 0x82 interrupt in */
  USB_EPT_DESC_INTERRUPT,                /* bmAttributes */
  LBYTE(USBD_CDC_CMD_MAXPACKET_SIZE),
  HBYTE(USBD_CDC_CMD_MAXPACKET_SIZE),    /* wMaxPacketSize */
  CDC_CUSTOM_HID_BINTERVAL_TIME,         /* bInterval */

  /* ------------- interface 1: cdc data ------------- */
  USB_DEVICE_IF_DESC_LEN,                /* bLength: interface descriptor size */
  USB_DESCIPTOR_TYPE_INTERFACE,          /* bDescriptorType: interface descriptor type */
  0x01,                                  /* bInterfaceNumber: interface 1 */
  0x00,                                  /* bAlternateSetting */
  0x02,                                  /* bNumEndpoints: 2 endpoints */
  USB_CLASS_CODE_CDCDATA,                /* bInterfaceClass: CDC data class code */
  0x00,                                  /* bInterfaceSubClass */
  0x00,                                  /* bInterfaceProtocol */
  0x00,                                  /* iInterface */

  USB_DEVICE_EPT_LEN,                    /* bLength: endpoint descriptor size */
  USB_DESCIPTOR_TYPE_ENDPOINT,           /* bDescriptorType: endpoint descriptor type */
  USBD_CDC_BULK_IN_EPT,                  /* bEndpointAddress: 0x81 bulk in */
  USB_EPT_DESC_BULK,                     /* bmAttributes */
  LBYTE(USBD_CDC_IN_MAXPACKET_SIZE),
  HBYTE(USBD_CDC_IN_MAXPACKET_SIZE),     /* wMaxPacketSize */
  0x00,                                  /* bInterval */

  USB_DEVICE_EPT_LEN,                    /* bLength: endpoint descriptor size */
  USB_DESCIPTOR_TYPE_ENDPOINT,           /* bDescriptorType: endpoint descriptor type */
  USBD_CDC_BULK_OUT_EPT,                 /* bEndpointAddress: 0x01 bulk out */
  USB_EPT_DESC_BULK,                     /* bmAttributes */
  LBYTE(USBD_CDC_OUT_MAXPACKET_SIZE),
  HBYTE(USBD_CDC_OUT_MAXPACKET_SIZE),    /* wMaxPacketSize */
  0x00,                                  /* bInterval */

  /* ------------- IAD: bind hid (if2) as its own function ------------- */
  USB_DEVICE_IAD_DESC_LEN,               /* bLength: IAD size 8 */
  USB_DESCIPTOR_TYPE_IAD,                /* bDescriptorType: interface association 0x0B */
  0x02,                                  /* bFirstInterface: 2 */
  0x01,                                  /* bInterfaceCount: 1 */
  USB_CLASS_CODE_HID,                    /* bFunctionClass: HID */
  0x00,                                  /* bFunctionSubClass: no boot */
  0x00,                                  /* bFunctionProtocol: none */
  0x00,                                  /* iFunction */

  /* ------------- interface 2: custom hid ------------- */
  USB_DEVICE_IF_DESC_LEN,                /* bLength: interface descriptor size */
  USB_DESCIPTOR_TYPE_INTERFACE,          /* bDescriptorType: interface descriptor type */
  0x02,                                  /* bInterfaceNumber: interface 2 */
  0x00,                                  /* bAlternateSetting */
  0x02,                                  /* bNumEndpoints: 2 endpoints */
  USB_CLASS_CODE_HID,                    /* bInterfaceClass: HID class code */
  0x00,                                  /* bInterfaceSubClass: no boot */
  0x00,                                  /* bInterfaceProtocol: none */
  0x00,                                  /* iInterface */

  0x09,                                  /* bLength: size of HID descriptor in bytes */
  HID_CLASS_DESC_HID,                    /* bDescriptorType: HID descriptor type */
  LBYTE(CDC_CUSTOM_HID_BCD_NUM),
  HBYTE(CDC_CUSTOM_HID_BCD_NUM),         /* bcdHID */
  0x00,                                  /* bCountryCode */
  0x01,                                  /* bNumDescriptors */
  HID_CLASS_DESC_REPORT,                 /* bDescriptorType: report descriptor type */
  LBYTE(sizeof(g_usbd_cdc_custom_hid_report)),
  HBYTE(sizeof(g_usbd_cdc_custom_hid_report)), /* wDescriptorLength */

  USB_DEVICE_EPT_LEN,                    /* bLength: endpoint descriptor size */
  USB_DESCIPTOR_TYPE_ENDPOINT,           /* bDescriptorType: endpoint descriptor type */
  USBD_CUSTOM_HID_IN_EPT,                /* bEndpointAddress: 0x83 interrupt in */
  USB_EPT_DESC_INTERRUPT,                /* bmAttributes */
  LBYTE(USBD_CUSTOM_IN_MAXPACKET_SIZE),
  HBYTE(USBD_CUSTOM_IN_MAXPACKET_SIZE),  /* wMaxPacketSize */
  CUSHID_BINTERVAL_TIME,                 /* bInterval */

  USB_DEVICE_EPT_LEN,                    /* bLength: endpoint descriptor size */
  USB_DESCIPTOR_TYPE_ENDPOINT,           /* bDescriptorType: endpoint descriptor type */
  USBD_CUSTOM_HID_OUT_EPT,               /* bEndpointAddress: 0x03 interrupt out */
  USB_EPT_DESC_INTERRUPT,                /* bmAttributes */
  LBYTE(USBD_CUSTOM_OUT_MAXPACKET_SIZE),
  HBYTE(USBD_CUSTOM_OUT_MAXPACKET_SIZE), /* wMaxPacketSize */
  CUSHID_BINTERVAL_TIME,                 /* bInterval */
};

/**
  * @brief usb hid report descriptor
  *        report layout kept identical to custom_hid demo:
  *        first byte = payload length, followed by up to 63 payload bytes
  */
#if defined ( __ICCARM__ ) /* iar compiler */
  #pragma data_alignment=4
#endif
ALIGNED_HEAD uint8_t g_usbd_cdc_custom_hid_report[USBD_CDC_CUSTOM_HID_SIZ_REPORT_DESC] ALIGNED_TAIL =
{
  0x06, 0xFF, 0x00,                      /* USAGE_PAGE(Vendor Page:0xFF00) */
  0x09, 0x01,                            /* USAGE (Demo Kit)               */
  0xa1, 0x01,                            /* COLLECTION (Application)       */
  /* 7 */

  /* Data OUT */
  0x09, 0x02,                            /*     USAGE                      */
  0x15, 0x00,                            /*     LOGICAL_MINIMUM (0)        */
  0x26, 0xFF,0x00,                       /*     LOGICAL_MAXIMUM (255)      */
  0x75, 0x08,                            /*     REPORT_SIZE (8)            */
  0x95, 0x40,                            /*     REPORT_COUNT (64)          */
  0x91, 0x02,                            /*     OUTPUT(Data,Var,Abs,Vol)   */
  /* 20 */

  /* Data IN */
  0x09, 0x03,                            /*     USAGE                      */
  0x15, 0x00,                            /*     LOGICAL_MINIMUM (0)        */
  0x26, 0xFF,0x00,                       /*     LOGICAL_MAXIMUM (255)      */
  0x75, 0x08,                            /*     REPORT_SIZE (8)            */
  0x95, 0x40,                            /*     REPORT_COUNT (64)          */
  0x81, 0x82,                            /*     INPUT(Data,Var,Abs,Vol)    */

  /* 33 */
  0xc0                                   /*     END_COLLECTION             */
};

/**
  * @brief usb hid descriptor(9 bytes, returned for HID_DESCRIPTOR_TYPE request)
  */
#if defined ( __ICCARM__ ) /* iar compiler */
  #pragma data_alignment=4
#endif
ALIGNED_HEAD uint8_t g_cdc_custom_hid_usb_desc[9] ALIGNED_TAIL =
{
  0x09,                                  /* bLength: size of HID descriptor in bytes */
  HID_CLASS_DESC_HID,                    /* bDescriptorType: HID descriptor type */
  LBYTE(CDC_CUSTOM_HID_BCD_NUM),
  HBYTE(CDC_CUSTOM_HID_BCD_NUM),         /* bcdHID: HID class specification release number */
  0x00,                                  /* bCountryCode: hardware target country */
  0x01,                                  /* bNumDescriptors: number of HID class descriptor to follow */
  HID_CLASS_DESC_REPORT,                 /* bDescriptorType: report descriptor type */
  LBYTE(sizeof(g_usbd_cdc_custom_hid_report)),
  HBYTE(sizeof(g_usbd_cdc_custom_hid_report)), /* wDescriptorLength: total length of report descriptor */
};

/**
  * @brief usb string lang id
  */
#if defined ( __ICCARM__ ) /* iar compiler */
  #pragma data_alignment=4
#endif
ALIGNED_HEAD static uint8_t g_string_lang_id[USBD_CDC_CUSTOM_HID_SIZ_STRING_LANGID] ALIGNED_TAIL =
{
  USBD_CDC_CUSTOM_HID_SIZ_STRING_LANGID,
  USB_DESCIPTOR_TYPE_STRING,
  0x09,
  0x04,
};

/**
  * @brief usb string serial
  */
#if defined ( __ICCARM__ ) /* iar compiler */
  #pragma data_alignment=4
#endif
ALIGNED_HEAD static uint8_t g_string_serial[USBD_CDC_CUSTOM_HID_SIZ_STRING_SERIAL] ALIGNED_TAIL =
{
  USBD_CDC_CUSTOM_HID_SIZ_STRING_SERIAL,
  USB_DESCIPTOR_TYPE_STRING,
};


/* device descriptor */
static usbd_desc_t device_descriptor =
{
  USB_DEVICE_DESC_LEN,
  g_usbd_descriptor
};

/* config descriptor */
static usbd_desc_t config_descriptor =
{
  USBD_CDC_CUSTOM_HID_CONFIG_DESC_SIZE,
  g_usbd_configuration
};

/* langid descriptor */
static usbd_desc_t langid_descriptor =
{
  USBD_CDC_CUSTOM_HID_SIZ_STRING_LANGID,
  g_string_lang_id
};

/* serial descriptor */
static usbd_desc_t serial_descriptor =
{
  USBD_CDC_CUSTOM_HID_SIZ_STRING_SERIAL,
  g_string_serial
};

static usbd_desc_t vp_desc;

/**
  * @brief  standard usb unicode convert
  * @param  string: source string
  * @param  unicode_buf: unicode buffer
  * @retval length
  */
static uint16_t usbd_unicode_convert(uint8_t *string, uint8_t *unicode_buf)
{
  uint16_t str_len = 0, id_pos = 2;
  uint8_t *tmp_str = string;

  while(*tmp_str != '\0')
  {
    str_len ++;
    unicode_buf[id_pos ++] = *tmp_str ++;
    unicode_buf[id_pos ++] = 0x00;
  }

  str_len = str_len * 2 + 2;
  unicode_buf[0] = (uint8_t)str_len;
  unicode_buf[1] = USB_DESCIPTOR_TYPE_STRING;

  return str_len;
}

/**
  * @brief  usb int convert to unicode
  * @param  value: int value
  * @param  pbus: unicode buffer
  * @param  len: length
  * @retval none
  */
static void usbd_int_to_unicode (uint32_t value , uint8_t *pbuf , uint8_t len)
{
  uint8_t idx = 0;

  for( idx = 0 ; idx < len ; idx ++)
  {
    if( ((value >> 28)) < 0xA )
    {
      pbuf[ 2 * idx] = (value >> 28) + '0';
  }
  else
  {
      pbuf[2 * idx] = (value >> 28) + 'A' - 10;
    }

    value = value << 4;

    pbuf[2 * idx + 1] = 0;
  }
}

/**
  * @brief  usb get serial number
  * @param  none
  * @retval none
  */
static void get_serial_num(void)
{
  uint32_t serial0, serial1, serial2;

  serial0 = *(uint32_t*)MCU_ID1;
  serial1 = *(uint32_t*)MCU_ID2;
  serial2 = *(uint32_t*)MCU_ID3;

  serial0 += serial2;

  if (serial0 != 0)
  {
    usbd_int_to_unicode (serial0, &g_string_serial[2] ,8);
    usbd_int_to_unicode (serial1, &g_string_serial[18] ,4);
  }
}

/**
  * @brief  get device descriptor
  * @param  none
  * @retval usbd_desc
  */
static usbd_desc_t *get_device_descriptor(void)
{
  return &device_descriptor;
}

/**
  * @brief  get device qualifier
  * @param  none
  * @retval usbd_desc
  */
static usbd_desc_t * get_device_qualifier(void)
{
  return NULL;
}

/**
  * @brief  get config descriptor
  * @param  none
  * @retval usbd_desc
  */
static usbd_desc_t *get_device_configuration(void)
{
  return &config_descriptor;
}

/**
  * @brief  get other speed descriptor
  * @param  none
  * @retval usbd_desc
  */
static usbd_desc_t *get_device_other_speed(void)
{
  return NULL;
}

/**
  * @brief  get lang id descriptor
  * @param  none
  * @retval usbd_desc
  */
static usbd_desc_t *get_device_lang_id(void)
{
  return &langid_descriptor;
}


/**
  * @brief  get manufacturer descriptor
  * @param  none
  * @retval usbd_desc
  */
static usbd_desc_t *get_device_manufacturer_string(void)
{
  vp_desc.length = usbd_unicode_convert((uint8_t *)USBD_DESC_MANUFACTURER_STRING, g_usbd_desc_buffer);
  vp_desc.descriptor = g_usbd_desc_buffer;
  return &vp_desc;
}

/**
  * @brief  get product descriptor
  * @param  none
  * @retval usbd_desc
  */
static usbd_desc_t *get_device_product_string(void)
{
  vp_desc.length = usbd_unicode_convert((uint8_t *)USBD_DESC_PRODUCT_STRING, g_usbd_desc_buffer);
  vp_desc.descriptor = g_usbd_desc_buffer;
  return &vp_desc;
}

/**
  * @brief  get serial descriptor
  * @param  none
  * @retval usbd_desc
  */
static usbd_desc_t *get_device_serial_string(void)
{
  get_serial_num();
  return &serial_descriptor;
}

/**
  * @brief  get interface descriptor
  * @param  none
  * @retval usbd_desc
  */
static usbd_desc_t *get_device_interface_string(void)
{
  vp_desc.length = usbd_unicode_convert((uint8_t *)USBD_CDC_CUSTOM_HID_DESC_INTERFACE_CDC_STRING, g_usbd_desc_buffer);
  vp_desc.descriptor = g_usbd_desc_buffer;
  return &vp_desc;
}

/**
  * @brief  get device config descriptor
  * @param  none
  * @retval usbd_desc
  */
static usbd_desc_t *get_device_config_string(void)
{
  vp_desc.length = usbd_unicode_convert((uint8_t *)USBD_CDC_CUSTOM_HID_DESC_CONFIGURATION_STRING, g_usbd_desc_buffer);
  vp_desc.descriptor = g_usbd_desc_buffer;
  return &vp_desc;
}

/**
  * @}
  */

/**
  * @}
  */

/**
  * @}
  */

#endif