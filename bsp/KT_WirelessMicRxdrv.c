...
//-----------------------------------------------------------------------------
static void KT_Bus_Write(uint16_t Register_Address, uint8_t Byte_Data,uint8_t chipNum)
{
   switch(chipNum)
   {
#ifdef DIVERSITY
    ...
    //    case 1: I2C_Byte_Write(KTWirelessMicRxw_addressBM,Register_Address,Byte_Data);break;
        case 0: I2C_Byte_Write(KT0656M_SLAVE_ADDR,Register_Address,Byte_Data);break;
        case 1: I2C_Byte_Write(KT0650_SLAVE_ADDR,Register_Address,Byte_Data);break;
      #else
       case 0: I2C_Byte_Write(KTWirelessMicRxw_addressAM,Register_Address,Byte_Data);break;
      #endif
#endif
...
//版    本：V0.1    For KT0656M                                                                    
//-----------------------------------------------------------------------------
static uint8_t KT_Bus_Read(uint16_t Register_Address,uint8_t chipNum)
{
    switch(chipNum)
    {
#ifdef DIVERSITY
    ...
    //    case 1: return( I2C_Byte_Read(KTWirelessMicRxr_addressBM, Register_Address) );break;
       case 0: return( I2C_Byte_Read(KT0656M_SLAVE_ADDR, Register_Address) );break;
       case 1: return( I2C_Byte_Read(KT0650_SLAVE_ADDR, Register_Address) );break;
       #else
        case 0: return( I2C_Byte_Read(KTWirelessMicRxr_addressAM, Register_Address) );break;
       #endif
#endif
...
// 1. 定义超时时间宏（可根据硬件实际需求调整，单位：ms）
#define MAX_POWER_ON_TIMEOUT  1000    // 电源启动最大等待时间（1秒）
#define MAX_CALI_TIMEOUT      500     // 校准过程最大等待时间（0.5秒）
#define DELAY_STEP            10      // 每次循环延迟步长（10ms，平衡响应速度与资源占用）

bool KT_WirelessMicRx_Init(void)
{
    uint8_t regx;
    uint32_t timeout;  // 超时计数器（用uint32_t避免溢出，适配长超时场景）

    // -------------------------------------------------------------------------
    // 2. 优化：电源启动等待循环（原死循环），添加超时判断
    // -------------------------------------------------------------------------
    timeout = 0;
    while (!(KT_Bus_Read(0x007F, chipSel) & 0x01))  // 等待power_on finish（bit0=1）
    {
        kt_delay_ms(DELAY_STEP);  // 延迟指定步长
        timeout += DELAY_STEP;    // 累加等待时间

        // 超时判断：超过最大等待时间，返回初始化失败
        if (timeout >= MAX_POWER_ON_TIMEOUT)
        {
            return 0;
        }
    }
    ...
    // 读取固件版本，验证芯片型号
    regx = KT_Bus_Read(0x010f, chipSel);
    if ((regx != 0x10) && (regx != 0x11))
        return 0;

    // 晶振选择配置
    regx = KT_Bus_Read(0x0108, chipSel);          
    KT_Bus_Write(0x0108, (regx & 0x1f) | (XTAL_SEL1 << 7) | (XTAL_SEL2 << 6) | (HLSI_INV << 5), chipSel);
    ...
    // DC_NOTCH_MUTE配置
    regx = KT_Bus_Read(0x0225, chipSel);
    KT_Bus_Write(0x0225, regx & 0xf1, chipSel);  // DC_NOTCH_MUTE_EN=1
#else
    // 非自动静音模式：DC_NOTCH_MUTE_EN=0
    regx = KT_Bus_Read(0x0225, chipSel);
    KT_Bus_Write(0x0225, regx & 0xf0, chipSel);
#endif
    ...
    // BPSK配置（原逻辑保留）
    regx = KT_Bus_Read(0x241, chipSel);          
    KT_Bus_Write(0x241, (regx & 0x3f) | (AUXDATA_EN << 7) | (BPSK_NEW_MODE << 6), chipSel);
    
    regx = KT_Bus_Read(0x243, chipSel);          
    KT_Bus_Write(0x243, (regx & 0x88) | (AUX_CARRY_NUM << 4) | CARRY_NUM_TH, chipSel);  // 载波数22，判决门限18
    
    regx = KT_Bus_Read(0x245, chipSel);          
    KT_Bus_Write(0x245, 0x22, chipSel);  // 连续正确/错误数据包数均为2
    ...
    // AFC（自动频率控制）配置
    regx = KT_Bus_Read(0x010e, chipSel);		  	
    if (KT_Bus_Read(0x010f, chipSel) == 0x10)  // A版本芯片
        KT_Bus_Write(0x010e, regx & 0xfe, chipSel);  // AFC CTRL FSM disable
    else  // B版本芯片
        KT_Bus_Write(0x010e, regx | 0x01, chipSel);   // AFC CTRL FSM enable

    regx = KT_Bus_Read(0x0087, chipSel);			
    KT_Bus_Write(0x0087, regx & ~0x08, chipSel);  // SOFT_AFC_MUTE=0

    regx = KT_Bus_Read(0x0217, chipSel); 
    KT_Bus_Write(0x0217, (regx & 0x3f) | (AFC_RNG << 6) | 0x01, chipSel);	// AFC范围：+/-60kHz
    KT_Bus_Write(0x0218, 0x02, chipSel);  // afc_en=1, AFC_FROZEN=0

    // 其他寄存器配置（原逻辑保留）
    regx = KT_Bus_Read(0x0256, chipSel); 
    KT_Bus_Write(0x0256, (regx & 0x8f) | 0x10, chipSel);  // comp_tc=1

    regx = KT_Bus_Read(0x002d, chipSel); 
    KT_Bus_Write(0x002d, regx | 0x08, chipSel);  // vtr_momitor_en=1

    regx = KT_Bus_Read(0x0010, chipSel); 
    KT_Bus_Write(0x0010, regx | 0x80, chipSel);  // ref_vtr_vth_sel=1
    ...
    regx = KT_Bus_Read(0x0133, chipSel);          
    KT_Bus_Write(0x0133, (regx | 0x40), chipSel);  // dll_rst_en=1

    regx = KT_Bus_Read(0x010e, chipSel); 
    KT_Bus_Write(0x010e, regx & 0xf7, chipSel);  // 关闭天线分集软件状态机（用硬件）

    // 音频输出选择配置
    regx = KT_Bus_Read(0x021c, chipSel); 
    KT_Bus_Write(0x021c, regx & 0xcf, chipSel);  // 输出合并信号

    // -------------------------------------------------------------------------
    // 3. 优化：第一次LOBAND校准等待循环（原死循环），添加超时判断
    // -------------------------------------------------------------------------
    regx = KT_Bus_Read(0x0108, chipSel);          
    KT_Bus_Write(0x0108, (regx | 0x10), chipSel);  // LOBAND_CALI_SCAN_EN=1（启动校准）

    uint32_t cali_timeout = 0;
    while (1)
    {
        kt_delay_ms(DELAY_STEP);
        cali_timeout += DELAY_STEP;

        // 检查校准是否完成（LOBAND_CALI_SCAN_EN=0）
        if ((KT_Bus_Read(0x0108, chipSel) & 0x10) == 0)
        {
            break;
        }

        // 超时判断：超过最大校准时间，返回失败
        if (cali_timeout >= MAX_CALI_TIMEOUT)
        {
            return 0;
        }
    }
...
#ifdef KT0655M
    // KT0655M芯片的LineIn配置（原逻辑保留）
    regx = KT_Bus_Read(0x003a, chipSel);
    KT_Bus_Write(0x003a, (regx & 0x1f), chipSel);	// LineIn相关电源打开
    kt_delay_ms(10);
    ...
    regx = KT_Bus_Read(0x0330, chipSel);
    KT_Bus_Write(0x0330, (regx & 0xc1) | (LINEIN_LOCAL_VOL_CTRL << 1), chipSel);	// LINEIN_LOCAL_VOL_CTRL

    regx = KT_Bus_Read(0x0334, chipSel);
    KT_Bus_Write(0x0334, (regx & ~0x18) | (PGA_GAIN_SEL << 3), chipSel);	// PGA_GAIN_SEL

    regx = KT_Bus_Read(0x0336, chipSel);
    KT_Bus_Write(0x0336, (regx & 0xc0) | (SLNC_MUTE_TIME << 1) | SLNC_MUTE_DIS, chipSel);	// SLNC_MUTE配置

    KT_Bus_Write(0x0337, (SLNC_MUTE_LOW_LEVEL << 4) | SLNC_MUTE_HIGH_LEVEL, chipSel);	// SLNC_MUTE_LEVEL

    KT_Bus_Write(0x0339, (ALC_DIS << 7) | ALC_VMAX, chipSel);	 // ALC配置
    ...
    // -------------------------------------------------------------------------
    // 4. 优化：第二次LOBAND校准等待循环（原死循环），添加超时判断
    // -------------------------------------------------------------------------
    regx = KT_Bus_Read(0x0108, chipSel);          
    KT_Bus_Write(0x0108, (regx | 0x10), chipSel);  // 再次启动LOBAND校准

    cali_timeout = 0;  // 重置超时计数器
    while (1)
    {
        kt_delay_ms(DELAY_STEP);
        cali_timeout += DELAY_STEP;
...
//-----------------------------------------------------------------------------
void KT_WirelessMicRx_Tune(long Freq) //470~960 MHz
{
...
//调用函数：KT_Bus_Read()、KT_Bus_Write()、kt_delay_ms()								
//全局变量：																		
