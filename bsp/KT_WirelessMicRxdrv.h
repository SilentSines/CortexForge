...
#define XTAL_24P576M_FREQ 0

#define HLSI_INV          0 //0: (HLSI); 1: inv(HLSI)

#define DCDC_EN           1 //DCDC使能位：0：关闭；1：使能。

#define AUTOMUTE_EN 						//undefine to disable automute    自动静音
...
#define FAST_RSSI_MUTE_EN       1     //1:enable 0:disable  用于控制快速接收信号强度指示（Fast RSSI）的静音功能是否启用。当该宏定义的值为 1 时，启用快速接收信号强度指示的静音功能；当值为 0 时，禁用该功能。
#define FAST_RSSI_PD_MUTE_EN    1     //1:enable 0:disable  用于控制快速接收信号强度指示（Fast RSSI）的功率下降静音功能是否启用。当该宏定义的值为 1 时，启用快速接收信号强度指示的功率下降静音功能；当值为 0 时，禁用该功能。
#define SUPER_RSSI_MUTE_EN      1     //1:enable 0:disable  用于控制超级接收信号强度指示（Super RSSI）的静音功能是否启用。当该宏定义的值为 1 时，启用超级接收信号强度指示的静音功能；当值为 0 时，禁用该功能
...
//#define AUTOMUTE_SNR_LOWTH	   130
//#define AUTOMUTE_SNR_HIGHTH	   148

#define AUTOMUTE_SNR_LOWTH	   130     //  输入-84 -10（衰减）对应值 -94   
#define AUTOMUTE_SNR_HIGHTH	   138     //  输入-80 -10（衰减）对应值 -90


//#define AUTOMUTE_SNR_LOWTH	   100     // 
//#define AUTOMUTE_SNR_HIGHTH	   118     //
...
//REG 0x0200 作用：补偿音量和音质
#define ADJUST_GAIN			  		1		   		//0:75KHz 1:50KHz 2:37.5KHz 3:25KHz 4:20KHz 5:120KHz 6:100KHz

//REG 0x0241
#define BPSK_NEW_MODE         1        	//	1:新的BPSK模式  0:旧的BPSK模式
#define AUXDATA_EN            1        	//	数字辅助信道使能，0：关闭数字辅助信道解调功能 1：使能数字辅助信道解调功能
...
//REG 0x026F 
//功能：啸叫抑制配置寄存器
//作用：用于控制啸叫抑制功能的使能和检测模式
#define SQUEAL_ELIM_EN        1    			//0：关闭啸叫抑制功能 1：使能啸叫抑制功能
#define SQEUAL_DET_EN         3	   			//0: 关闭		1: 保留		2: 保留 3:开启	bit4:3 //b00：关闭啸叫检测。b11：使能啸叫检测。
#define FNOTCH_NOTRESP_TH     1    			//0: 1/4;   1: 1/2
#define N_OCTAVE              3					//0: 1/5;   1: 1/10;  2: 1/20;	3:1/80
...
void rfIntCtl(void);
#ifdef XTAL_DUAL
static void selXtal(char xtalSel);
#endif
...
// 晶振
// bit7 		XTAL_SEL  0:XO1 1:XO2
// bit6~0 	保留
#define AND_CFG (0x0012)
// DC to DC使能位
// bit7~3 	保留
// bit2 		DCDC_EN 0:关闭 ；1：使能
// bit1~0 	保留
#define AND_CFG38 (0x002D)
// 接收频率设置,<19:12>
#define CHAN_CFG0 (0x0045)
// 接收频率设置,<11:4>
#define CHAN_CFG1 (0x0046)
// bit7~4 接收频率设置,<3:0>
// bit3~1 保留位
// bit0 	 CHAN_VALID 置1写入频率，然后自动清0
#define CHAN_CFG2 (0x0047)
// bit7 		保留位
// bit6 		SAI_SLAVE_SYNC_EN 配置完成SAI_SLAVE_MODE和SAI_SLAVE_DSIZE两个控制位后，该位置1
// bit5 		STEREO_MONO_SEL		0：单声道 1：立体声
// bit4 		LR_SEL
// bit3~2 	SAI_SLAVE_MODE 		b00:保留；	b01：IIS模式；b10：左对齐；	b11：右对齐
// bit1~0 	SAI_SLAVE_DSIZE		传输数据的带宽 b00:16bit；	b01：20bit；	b10：24bit；	b11：8bit
#define SAI_SLAVE_CFG (0x0050)
// bit7~4 保留
// bit3 	MASTER_MCLK_LRCLK_RATIO	 串口音频接口作为主设备时，MCLK/LRCLK频率比。0：MCLK/LRCLK =128；1：MCLK/LRCLK =256
// bit2 	MASTER_MCLK_SEL		       串口音频接口作为主设备时MCLK频率选择，芯片内部SAI master 输出给外部slave工作时钟MCLK,0：12MHZ;1:24MHZ
// bit1~0 传输模式 00和01：I2S/IIS模式，10：左对齐；11：右对齐
#define SAI_MASTER_CFG (0x0051)

#define SAI_INTERFACE_EN (0x0052)

#define INT_EN (0x0055)
#define INT_FLAG (0x0059)
#define PLL_FAM_CTRL5 (0x0061)
#define POWER_STA_CTRL1 (0x007F)
#define AUTOMUTE_FSM_STA (0x0088)
#define ADC_CHAN0_HB (0x00C0)
#define ADC_CHAN0_LB (0x00C1)

#define SW_CFG0 (0x0100) // bit7:接收不到导频使能静音 bit5:
#define SW_CFG5 (0x0105)
#define SW_CFG6 (0x0106)
#define SW_CFG8 (0x0108)
#define SW_CFG14 (0x010E)
#define SW_CFG15 (0x010F)

#define MANUFACTURER_ID0 (0x0192)
#define MANUFACTURER_ID1 (0x0193)

#define TOP_CFG0 (0x0200)
#define TOP_CFG1 (0x0201)
#define TOP_STATUS0 (0x0209)
#define TOP_STATUS1 (0x020A)
#define TOP_STATUS3 (0x020C)
#define TOP_STATUS4 (0x020D)

#define AFC_CFG0 (0x0217)
#define AFC_CFG1 (0x0218)
#define AFC_STATUS0 (0x0219)
#define AFC_STATUS1 (0x021A)
#define ANTDIV_CFG1 (0x021C)

#define AUDIOLINK_CFG (0x0224)
#define AUTOMUTE_CFG0 (0x0225)
#define BPSK_CFG0 (0x0241)
#define BURST_DATAH (0x0246)
#define BURST_DATAL (0x0247)

#define AUX_DATA_AH (0x0248)
#define AUX_DATA_AL (0x0249)
#define AUX_DATA_BH (0x024A)
#define AUX_DATA_BL (0x024B)
#define AUX_DATA_CH (0x024C)
#define AUX_DATA_CL (0x024D)
#define AUX_DATA_DH (0x024E)
#define AUX_DATA_DL (0x024F)

#define DE_EXP_CFG (0x0256)
#define EQ_CFG0 (0x0257)
#define EQ_CFG1 (0x0258)
#define EQ_CFG2 (0x0259)
#define EQ_CFG3 (0x025A)
#define EQ_CFG4 (0x025B)
#define EQ_CFG5 (0x025C)
#define EQ_CFG6 (0x025D)
#define EQ_CFG7 (0x025E)
#define EQ_CFG8 (0x025F)
#define EQ_CFG9 (0x0260)
#define EQ_CFG10 (0x0261)
#define EQ_CFG11 (0x0262)
#define EQ_CFG12 (0x0263)
#define EQ_CFG13 (0x0264)
#define EQ_CFG14 (0x0265)

#define ECHO_CFG0 (0x0266)
#define ECHO_CFG1 (0x0267)
#define ECHO_CFG2 (0x0268)
#define EXCITER_CFG0 (0x0269)
#define EXCITER_CFG1 (0x026A)
#define SQUEAL_CFG0 (0x026F)

#define RSSI TOP_STATUS3

#define PILOT_SIGNAL_VALID 0xC0 // 有效导频信号标识(0xC0表示检测到有效信号)
#define AUX_DATA_START_ADDR 0x0248 // 辅助数据寄存器起始地址
#define MAX_AUX_DATA_COUNT 4 // 辅助数据组数
...
#define ADDR_PIN 1
#define KT0656M_SLAVE_ADDR  (0x31 | (ADDR_PIN << 3))
#define KT0650_SLAVE_ADDR   (0x35 | (ADDR_PIN << 3))
